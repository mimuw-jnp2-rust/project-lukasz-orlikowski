use gloo_net::Error;
use yew::{html, Component, Context, Html, MouseEvent, function_component};
use gloo_storage::{LocalStorage, Storage};
use crate::Route;
use crate::api::{get_user_teams, create_team_board};
...
impl Component for TeamBoardCreate {
        ...
        match msg {
            Self::Message::Submit => {
                ctx.link().send_future(async move {
                    ...
                    let res = create_team_board(name.as_str(), team.parse::<i32>().unwrap(), token.as_str()).await;
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        if self.teams.is_none() {
            ...
            let teams = teams_copy.unwrap().into_iter().map(|team| html! {
                        <TeamDetails id={team.id.clone()} owner={team.owner.clone()} name={team.name.clone()} />
                    });
            match self.token {
